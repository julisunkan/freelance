...
# Routes are /sign/a|b/<token> — /nda prefix added by DispatcherMiddleware

@signing_bp.route('/sign/a/<token>')
def sign_a(token):
    nda = models.get_nda_by_token(token)
    if not nda or nda['sign_token_a'] != token:
        abort(404)
    if nda['status'] == 'signed':
        return render_template('sign.html', nda=nda, party='a',
                               already_signed=True, locked=True)
    if nda['signature_a']:
        return render_template('sign.html', nda=nda, party='a',
                               already_signed=True, locked=False)
    models.log_audit(nda['id'], 'Party A viewed signing page',
                     _get_ip(), request.user_agent.string)
    if nda['status'] == 'draft':
        models.update_nda_status(nda['id'], 'sent')
    return render_template('sign.html', nda=nda, party='a',
                           already_signed=False, locked=False)


@signing_bp.route('/sign/b/<token>')
def sign_b(token):
    nda = models.get_nda_by_token(token)
    if not nda or nda['sign_token_b'] != token:
        abort(404)
    if nda['status'] == 'signed':
        return render_template('sign.html', nda=nda, party='b',
                               already_signed=True, locked=True)
    if nda['signature_b']:
        return render_template('sign.html', nda=nda, party='b',
                               already_signed=True, locked=False)
    models.log_audit(nda['id'], 'Party B viewed signing page',
                     _get_ip(), request.user_agent.string)
    if nda['status'] == 'draft':
        models.update_nda_status(nda['id'], 'sent')
    return render_template('sign.html', nda=nda, party='b',
...
@signing_bp.route('/sign/a/<token>', methods=['POST'])
def submit_sign_a(token):
    ...
    view_url = url_for('main.view_nda', public_id=nda['public_id'])
    return jsonify({'ok': True, 'status': updated['status'],
                    'public_id': nda['public_id'], 'view_url': view_url})


@signing_bp.route('/sign/b/<token>', methods=['POST'])
